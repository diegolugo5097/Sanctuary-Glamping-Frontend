...
const Form = ({ currentId, setCurrentId }) => {
  ...
  const galleries = useSelector((state) =>
    currentId ? state.galleries.find((c) => c._id === currentId) : null
  );

  const dispatch = useDispatch();

  useEffect(() => {
    if (galleries) setPostData(galleries);
  }, [galleries]);
  ...
  const handleSummit = async (e) => {
    ...
    if (postData.image === "") {
      ...
      if (currentId) {
        dispatch(updateGallery(currentId, postData));
        clear();
        return Swal.fire({
          position: "top-end",
          icon: "success",
        ...
        dispatch(createGallery(postData));
        clear();
        return Swal.fire({
          position: "top-end",
          icon: "success",
  ...
  return (
    <>
      ...
      <Modal
        title={currentId ? "Editar Galeria" : "Crear Galeria"}
        onCancel={onClose}
